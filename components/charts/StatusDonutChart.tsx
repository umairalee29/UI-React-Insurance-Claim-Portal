...
export function StatusDonutChart({ byStatus, total }: Props) {
  const data = Object.entries(byStatus).map(([status, count]) => ({
    ...
    value: count,
    fill: STATUS_META[status]?.color ?? '#9ca3af',
    color: STATUS_META[status]?.color ?? '#9ca3af',
  }))

  return (
    <div className="flex items-center gap-8 px-6 py-5">
      <div className="relative shrink-0 w-44 h-44 [&_*]:outline-none [&_*]:focus:outline-none">
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div style={{
                    ...
                    <span style={{ fontSize: 13, fontWeight: 500, color: '#374151' }}>{entry.name}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: '#111827', fontVariantNumeric: 'tabular-nums' }}>{entry.value}</span>
                    <span style={{ fontSize: 11, color: '#9ca3af', fontVariantNumeric: 'tabular-nums' }}>{pct}%</span>
                  </div>
                )
              }}
            />
