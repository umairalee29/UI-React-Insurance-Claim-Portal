...
interface PopulatedClaim extends Omit<
  IClaim,
  "claimantId" | "assignedAdjusterId"
> {
  claimantId: IUser;
...
export default function QueuePage() {
  ...
  async function confirmAssign() {
    ...
    try {
      const res = await fetch("/api/admin/assign", {
        ...
        body: JSON.stringify({
          claimId: String(pendingClaim._id),
  ...
  async function bulkAssignToMe() {
    ...
    try {
      ...
      toast.success(
        `${selected.size} claim${selected.size !== 1 ? "s" : ""} assigned to you`,
      );
      setSelected(new Set());
  ...
  function handleSort(field: string) {
    setFilters((f) => {
      if (f.sortBy !== field)
        return { ...f, sortBy: field, sortOrder: "desc", page: 1 };
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex flex-wrap gap-3">
          ...
          <div ref={statusRef} className="relative">
            ...
            >
              ...
              <svg
                className={`ml-auto w-3.5 h-3.5 text-gray-400 transition-transform ${statusOpen ? "rotate-180" : ""}`}
                viewBox="0 0 12 12"
                fill="none"
              >
                <path
              ...
              <div className="absolute z-20 mt-1.5 left-0 bg-white border border-gray-100 rounded-xl shadow-lg py-1 min-w-[160px]">
                ...
                {(
                  Object.entries(STATUS_CONFIG) as [
                    ClaimStatus,
                    (typeof STATUS_CONFIG)[ClaimStatus],
                  ][]
                ).map(([key, cfg]) => (
                  <button
                    key={key}
              ...
              <tr className="bg-gray-50">
                <th className="px-4 py-3 text-left">
                  <input
                    type="checkbox"
                    checked={
                      selected.size === claims.length && claims.length > 0
                    }
                    onChange={toggleAll}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        ) : String(claim.assignedAdjusterId._id) ===
                          session?.user?.id ? (
        ...
        <div className="flex flex-col items-center text-center gap-4 py-2">
          ...
          <div>
            <h3 className="text-base font-semibold text-gray-900">
              Assign {selected.size} claim{selected.size !== 1 ? "s" : ""} to
              yourself?
            </h3>
